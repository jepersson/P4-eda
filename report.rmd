...
recession on active loan volumes let's plot the change in loan volumes for the
entire period. We will use the monthly deltas to make the graph easier to read.

```{r echo=FALSE, warning=FALSE, fig.align="center", fig.height = 9, fig.width = 9, Bivariate_Plots_0}
...
```{r echo=FALSE, warning=FALSE, fig.align="center", fig.height=3, fig.width=9, Bivariate_Plots_1}
...
# Ratio for active loan's current statuses over time.
ggplot(subset(loan_data, 
              LoanOriginationDate < as.Date("2014-03-01") & 
                LoanOriginationDate > as.Date("2005-12-01") &
                is.na(ClosedDate))) +
  geom_histogram(aes(LoanOriginationDate, 
                     fill = LoanStatus),
                 bins = 100,
                 position = "fill")
...
```{r echo=FALSE, warning=FALSE, fig.align="center", fig.height=3, fig.width=9, Bivariate_Plots_2}

# Plotting original loan amount vs the given borrower rate
ggplot(loan_data) +
  geom_point(aes(LoanOriginalAmount,
                 BorrowerRate),
             alpha = 0.1,
             size = 0.4)
...
# Plotting borrower rate vs origination date
ggplot(loan_data) +
  geom_point(aes(LoanOriginationDate,
                 BorrowerRate),
             alpha = 0.1,
             size = 0.4)

# Plotting borrower rate vs debt to income ratio.
ggplot(loan_data) +
  geom_point(aes(DebtToIncomeRatio,
                 BorrowerRate),
             alpha = 0.1,
             size = 0.4)
...
# Plotting ListingCategory vs borrower rate
ggplot(loan_data) +
  geom_boxplot(aes(ListingCategory,
                 BorrowerRate)) +
  coord_flip()

# Plotting ListingCategory vs borrower rate
ggplot(loan_data) +
  geom_point(aes(EmploymentStatusDuration,
                 BorrowerRate),
             alpha = 0.1,
             size = 0.4)

# Plotting Occupation vs Borrower rate
ggplot(loan_data) +
...
# Plotting number of investors vs borrower rate
ggplot(loan_data) +
  geom_point(aes(Investors,
                 BorrowerRate),
             alpha = 0.1,
             size = 0.4)
